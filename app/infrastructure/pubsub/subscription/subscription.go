...
func New(
	subscriptionName string,
	processMessage func(ctx context.Context, sub string, m *pubsub.Message) (statusCode int, err error),
	recieveWithSettings Receive) *SubscriptionWrapper {
	return &SubscriptionWrapper{subscriptionName: subscriptionName, processMessage: processMessage, recieveWithSettings: recieveWithSettings}
}

func (s *SubscriptionWrapper) Start() (*SubscriptionWrapper, error) {

	if s.stop {
		return &SubscriptionWrapper{}, nil
	}
...
func (s *SubscriptionWrapper) WithPushHandler(
	path string,
	e server.EchoWrapper) *SubscriptionWrapper {
	e.POST(path, s.pushHandler)
	return s
}
